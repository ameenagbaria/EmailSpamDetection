...
def main():
    ...
    X_train_metadata = np.array([extract_metadata(x) for x in X_train])
    X_test_metadata = np.array([extract_metadata(x) for x in X_test])
    ...
    # Convert metadata to sparse matrices to allow for stacking with TF-IDF features
    X_train_metadata = csr_matrix(X_train_metadata)
    X_test_metadata = csr_matrix(X_test_metadata)

    # Preprocess text
    X_train = [preprocess(x) for x in X_train]
    X_test = [preprocess(x) for x in X_test]

    y_train = np.array(train_split["category_id"])
    y_test = np.array(test_split["category_id"])
    ...
    # Concatenate TF-IDF features with metadata (using sparse hstack for sparse matrices)
    X_train_combined = csr_matrix(hstack([X_train, X_train_metadata]))
    X_test_combined = csr_matrix(hstack([X_test, X_test_metadata]))
